...
- [x] **실측 성능(RTINGS 등)**(`measurement` 테이블, `scripts/load_measurements.py`) — HDR 실측 피크·입력랙·DCI-P3·명암비. 6개 플래그십(U8Q 4300nit·S95F 3789nit·QM8K 2752nit 등), 출처(source) 표기. API `GET /api/measurements?model=`. ※DisplaySpecifications 봇차단·TechSpecs API 키필요로 미연동(tuner/vesa_mm 컬럼만 준비).
