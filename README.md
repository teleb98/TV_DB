...
- [x] **브랜드 마케팅 feature(우선순위)**(`model_feature` 테이블, `scripts/load_features.py`) — 스펙 외 브랜드가 앞세우는 기능을 **제품페이지 노출 순서(rank1=최상단=가장 중요)**로 적재. 각 브랜드 공식 도메인 확인(samsung.com/lg.com/tcl.com/hisense-usa.com, Sony는 크롤차단→마케팅표기). 예: QN90F 1위=Vision AI·G5 1위=Brightness Booster·U8Q 1위=MiniLED Pro 5000nit. API `GET /api/features?model=`.
