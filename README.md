...
- [x] **실측 성능(RTINGS 등)**(`measurement` 테이블, `scripts/load_measurements.py`) — HDR 실측 피크·입력랙·DCI-P3·rec2020·명암비. **30종**(2024~2026 플래그십~엔트리 망라, QM9K 5413 → B5 688nit / 상위·플래그십 티어 커버리지 ~46%), 출처(source) 표기. API `GET /api/measurements?model=`(밝기순 랭킹). ※DisplaySpecifications 봇차단·TechSpecs API 키필요로 미연동(tuner/vesa_mm 컬럼만 준비).
